...
uv run experiments/exp1i_tdc_ablations.py
uv run experiments/exp2_synthetic_acr.py
```

Outputs land in `results/exp{0,1,1b,1c,1e,1f,1g,1h,1f_tdc,1h_tdc,1i_tdc,2}/`. See [EXPERIMENTS.md](EXPERIMENTS.md) for the full design / observations / conclusions.
...
- **exp1i-tdc** — Two ablations on the broken TDC recipe: drop reversal-augmentation, and use scaffold-test inputs as the unlabelled pool (true transductive). Recovers a 5–8 % SSL benefit at low labels but not at high n.
